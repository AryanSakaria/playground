...
def get_time_embedding(time_steps, t_emb_dim):
	factor = 10000 ** ((torch.arange(
		start = 0, dtype= torch.float32, end = t_emb_dim//2, device = time_steps.device)/ (t_emb_dim //2) 
		))
...
class DownBlock(nn.Module):
	def __init__(self, in_channels, out_channels, t_emb_dim, down_sample=True, num_heads=4):
		super().__init__()
	...
	def forward(self, x, t_emb):
		out = x
		#Loop from here for multi layer
		resnet_input = out
		out = self.resnet_conv_first(out)
		out = out + self.t_emb_layers(t_emb)[:,:,None, None] # see why do this. Add breakpoint and figure out shapes
		...
		in_attn = in_attn.transpose(1, 2) # understand shapes
...
class MidBlock(nn.Module):
	def __init__(self, in_channels, out_channels, t_emb_dim, num_heads=4):
		super().__init__()
		#why two instances of every resnet block?
		...
		self.attention_norm = nn.GroupNorm(8, out_channels)
		self.attention = nn.MultiheadAttention(out_channels, num_heads, batch_first = True)# what is batch first?
	...
	def forward(self, x, t_emb):
		...
		out = out + self.t_emb_layers[0](t_emb)[:,:,None,None] #huh, what?
		out = self.resnet_conv_second[0](out)
		out = out + self.residual_input_conv[0](resnet_input)

		#for multi layer start for loop here
		#attention block
		batch_size, channels, h, w = out.shape 
		in_attn = out.reshape(batch_size, channels, h*w)
		in_attn = self.attention_norm(in_attn)
		in_attn = in_attn.transpose(1,2) #why??
		out_attn, _ = self.attention(in_attn, in_attn, in_attn)
		out_attn = out_attn.transpose(1,2).reshape(batch_size, channels, h, w)
		out = out + out_attn
...
class UpBlock(nn.Module):
	def __init__(self, in_channels, out_channels, t_emb_dim, up_sample= True, num_heads=4):
		super().__init__()
...
class Unet(nn.Module):
	def __init__(self, im_channels):
		...
		#called once to get initial time step representaion
